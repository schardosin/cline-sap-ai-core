...
To install from the source code do the following:
```bash
npm run install:all
vsce package
code --install-extension claude-dev-x.x.xx.vsix
```
* Make sure vsce is installed in your computer
* Replace the x.x.xx by the correct version found in the file generated by vsce package
